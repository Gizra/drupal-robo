...
trait AutoUpdateTrait {
  ...
  public function updateModules() {
    ...
    foreach ($data as $project) {
      ...
      $derived = $this->deriveComposerConstraint($project['recommended']);
      $version = $derived['constraint'];
      $major = $derived['major'];
      ...
      $current_branch = trim((string) shell_exec('git symbolic-ref --short HEAD'));
      ...
      $lock_changed = trim((string) shell_exec('git status --porcelain composer.lock'));
      if (!$lock_changed) {
  ...
  /**
   * Derive a Composer version constraint from a Drupal recommended version.
   *
   * Version numbers in Drupal can take several patterns; all of them map to a
   * Composer constraint:
   *   4.0.0         => 4.0
   ...
   *   The recommended version as reported by the update module.
   *
   * @return array{constraint: string, major: string|null}
   *   The derived constraint (without the leading caret) and the major version,
   *   or the untouched input plus a NULL major when it does not match.
   */
  protected function deriveComposerConstraint(string $recommended): array {
    $constraint = $recommended;
    $major = NULL;
    if (preg_match('/^(?:\d+\.x-)?(\d+)\.(\d+)(?:\.(\d+))?(?:-([A-Za-z0-9.-]+))?$/', trim($recommended), $matches)) {
      // e.g., '8' or '2'.
      $major = $matches[1];
      // e.g., '7' or '0'.
      $minor = $matches[2];
      ...
      if ($suffix) {
        ...
        $constraint .= "@{$cleaned_suffix}";
      }
    }
    return ['constraint' => $constraint, 'major' => $major];
  }
